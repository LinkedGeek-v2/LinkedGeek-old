Steps:

1) Change Connection String
2) Run solution and update Database
3) Open SqlData Folder and run the scripts with the order they are numbered! (Make sure database name is correct
on 1st line of scripts)
4)Every account Username is the email and password is Password1!
5) Log in with Lee Simmons to check chat history
6) Registering as new user or loging in with google/facebook now doesnt work because after merges user isnt saved on database as company/developer.
7) There is one logic bug in save details form in javascript.
